...
// fakeRelease serves a GitHub "latest release" with an Maple build for
// this OS and its SHA256SUMS.txt.
func fakeRelease(t *testing.T, tag string, exe []byte, corrupt bool) *httptest.Server {
	t.Helper()
	name := fmt.Sprintf("Maple-%s-%s-%s.exe", tag, goruntime.GOOS, goruntime.GOARCH)
...
func updateService(t *testing.T, version, api string) (*Service, string) {
	t.Helper()
	dir := t.TempDir()
	exe := filepath.Join(dir, "Maple.exe")
